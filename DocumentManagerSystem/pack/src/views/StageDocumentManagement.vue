<template>
  <div>
    ...
    <v-dialog v-model="versionUpdateDialog" hide-overlay persistent width="450">
      <v-card>
        <v-card-title class="blue darken-2 white--text">文件更版</v-card-title>
        <v-container grid-list-sm>
          <v-layout row wrap>
            <ValidationObserver ref="uploadVersionForm" style="width:100%">
              <v-flex xs12>
                <ValidationProvider name="DocumentName" rules="required" v-slot="{ errors }">
                  <v-text-field
                    dense
                    :error-messages="errors[0]"
                    v-model="VersionEdit.DocumentName"
                    label="文件名稱"
                  ></v-text-field>
                </ValidationProvider>
              </v-flex>
              <v-flex xs12>
                <ValidationProvider name="DocumentType" rules="required" v-slot="{ errors }">
                  <v-select
                    ...
                    item-text="Name"
                    item-value="Id"
                    v-model="VersionEdit.DocumentType"
                    :label="$t('文件階層')"
                    persistent-hint
                    @change="ChangeVersionEditDocumentTypes"
                  ></v-select>
                </ValidationProvider>
              </v-flex>
              <v-flex xs12>
                <ValidationProvider name="DocumentNumber" rules="required" v-slot="{ errors }">
                  <v-text-field
                    dense
                    :error-messages="errors[0]"
                    v-model="VersionEdit.DocumentNumber"
                    label="文件編號"
                  ></v-text-field>
                </ValidationProvider>
              </v-flex>
              <template v-if="!IsSpecialDocument(VersionEdit.DocumentNumber)">
                <v-flex xs12>
                  <ValidationProvider name="DocumentVersion" rules="required" v-slot="{ errors }">
                    <v-text-field
                      dense
                      :error-messages="errors[0]"
                      v-model="VersionEdit.DocumentVersion"
                      label="版號"
                      prefix="v"
                      hint="請輸入#.#.#格式 Ex. 1.0.0"
                    ></v-text-field>
                ...
                <v-flex xs8>
                  <v-menu
                    v-model="isShowEffectiveDatePicker_Version"
                    :close-on-content-click="false"
                    transition="scale-transition"
                    offset-y
                  >
                    <template v-slot:activator="{ on }">
                      <ValidationProvider name="EffectiveDate" rules="required" v-slot="{ errors }">
                        <v-text-field
                          :error-messages="errors[0]"
                          v-model="VersionEditEffectiveDate"
                          :label="'上版日期'"
                          readonly
                          v-on="on"
                        ></v-text-field>
                    ...
                    <v-date-picker
                      ...
                      :title-date-format="yearFormat"
                      :header-date-format="yearFormat"
                      v-model="VersionEdit.EffectiveDate"
                      locale="Language"
                      @input="isShowEffectiveDatePicker_Version = false"
                    ></v-date-picker>
                  </v-menu>
                </v-flex>
              </template>
              <v-flex xs12>
                <ValidationProvider name="DocumentOwner" rules="required" v-slot="{ errors }">
                  <v-text-field
                    dense
                    :error-messages="errors[0]"
                    v-model="VersionEdit.DocumentOwner"
                    label="權責人員"
                  ></v-text-field>
                </ValidationProvider>
              </v-flex>
              <v-flex xs12>
                <v-file-input
                  ...
                  type="file"
                  name="files"
                  v-model="previewVersionEditFilesData"
                  @change="previewVersionEditFiles"
                ></v-file-input>
              </v-flex>
            </ValidationObserver>
          </v-layout>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn type="submit" color="red" dark @click="versionUpdateDialog = false">取消</v-btn>
          <v-btn type="submit" color="primary" dark @click="setDocumentVersion">更版</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog v-model="documentVersionListDialog" hide-overlay width="1280">
      <v-card>
        <v-card-title class="blue darken-2 white--text">更版歷程</v-card-title>
        <v-container fluid grid-list-md>
          <v-layout wrap>
            <v-flex flex-grow-1 xs12 sm12 md12 lg12 x12>
              <v-card>
                <v-data-table
                  :loading="versionHistoryIsLoading"
                  :headers="versionHistoryHeaders"
                  :items="DocumentVersionList"
                  :items-per-page.sync="pagination.rowsPerPage"
                  :footer-props="{
                                          'items-per-page-options': [10, 20], // -> Add this example
                                          'items-per-page-text':'每頁筆數'
                                      }"
                  :sort-by="'DocumentNumber'"
                  class="elevation-1"
                ...
                >
                  ...
                  <template
                    v-slot:item.EffectiveDate="{ item }"
                  >{{ formatDate(item.EffectiveDate) }}</template>
                  <template v-slot:item.UpdateTime="{ item }">{{ formatDate(item.UpdateTime) }}</template>
                  <template v-slot:item.action="{ item }">
                    <v-btn
                      small
                      tile
                      outlined
                      color="primary"
                      @click="downloadDocumentVersion(item)"
                    >
                      <v-icon left style="cursor:pointer;">archive</v-icon>下載
                    </v-btn>
                  </template>
    ...
    <v-dialog v-model="IsShowInvalidPopup"  hide-overlay persistent width="300">
      <v-card>
        <v-card-title class="blue darken-2 white--text">請選擇作廢日期</v-card-title>
        <v-card-actions>
          <v-container grid-list-sm>
            <v-layout row wrap>
              <v-flex xs12>
                <v-menu
                  v-model="isShowInvalidDatePicker"
                  :close-on-content-click="false"
                  transition="scale-transition"
                  offset-y
                >
                  <template v-slot:activator="{ on }">
                    <ValidationProvider name="InvalidDate" rules="required" v-slot="{ errors }">
                      <v-text-field
                        v-model="formatInvalidDate"
                        :label="'作廢日期'"
                        readonly
                        :error-messages="errors[0]"
                        v-on="on"
                  ...
                  <v-date-picker
                    ...
                    v-model="InvalidDate"
                    locale="Language"
                    @input="isShowInvalidDatePicker = false"
                  ></v-date-picker>
                </v-menu>
              </v-flex>
            </v-layout>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn type="submit" color="red" dark @click="CancelInvalidPopup">取消</v-btn>
          <v-btn type="submit" color="primary" dark @click="InvalidDocument">確定</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  methods: {
    ...
    changeVersion: function (item) {
      this.VersionEdit.Id = item.Id
      this.VersionEdit.EffectiveDate =
        item.EffectiveDate === '0001-01-01T00:00:00'
          ? ''
          : item.EffectiveDate.split('T')[0]
      this.VersionEdit.PublishDate = item.PublishDate
      this.VersionEdit.DocumentType = item.DocumentType
      this.VersionEdit.DocumentName = item.DocumentName
      this.VersionEdit.DocumentNumber = item.DocumentNumber
      this.VersionEdit.DocumentVersion = item.DocumentVersion
      this.VersionEdit.DocumentOwner = item.DocumentOwner

      this.VersionEdit.OldDocumentVersion = item.DocumentVersion
      this.VersionEdit.OldEffectiveDate = this.VersionEdit.EffectiveDate

      this.versionUpdateDialog = true
    },
    versionHistory: function (item) {
      this.GetDocumentVersionList(item)
    },
    edit: function (item) {
    ...
    setDocumentVersion: async function () {
      let self = this

      const isValid = await self.$refs.uploadVersionForm.validate()

      if (!isValid) return

      if (
        this.VersionEdit.OldDocumentVersion === this.VersionEdit.DocumentVersion
      ) {
        self.AlertDialogMessage = '請更新版號'
        self.IsOpenAlertDialog = true
        setTimeout(() => {
          self.IsOpenAlertDialog = false
        }, 1000)
      ...
      if (this.VersionEdit.OldDocumentVersion > this.VersionEdit.DocumentVersion) {
        self.AlertDialogMessage = '版號必須大於舊版號，請確認!'
        self.IsOpenAlertDialog = true
        setTimeout(() => {
          self.IsOpenAlertDialog = false
        }, 1000)
      ...
      var newEffDate = new Date(this.VersionEdit.EffectiveDate)
      var oldEffDate = new Date(this.VersionEdit.OldEffectiveDate)

      if (newEffDate < oldEffDate) {
        self.AlertDialogMessage = '生效日期必須大於舊生效日期'
        self.IsOpenAlertDialog = true
        setTimeout(() => {
          self.IsOpenAlertDialog = false
        }, 1000)
      ...
      let uploadFile = this.VersionEdit.currentUploadFile

      if (uploadFile == null) {
        self.AlertDialogMessage = '請選擇新檔案'
        self.IsOpenAlertDialog = true
        setTimeout(() => {
          self.IsOpenAlertDialog = false
        }, 1000)
      ...
      let effectiveDate = this.VersionEdit.EffectiveDate
      let publishDate = this.VersionEdit.PublishDate
      let documentType = this.VersionEdit.DocumentType
      let documentName = this.VersionEdit.DocumentName
      let documentNumber = this.VersionEdit.DocumentNumber
      let DocumentVersion = this.VersionEdit.DocumentVersion
      let DocumentOwner = this.VersionEdit.DocumentOwner
      let id = this.VersionEdit.Id
      ...
      axios
        .post('/api/Document/ChangeDocumentVersion', formData, {
          headers: {
            'content-type': 'multipart/form-data'
          }
        })
        ...
        .finally(function () {
          self.loadingDialog = false
          self.versionUpdateDialog = false
        })
    },
    ...
    OpenInvalidPopup: function (item) {
      this.IsShowInvalidPopup = true
      this.InvalidDoc = item
      // if (confirm('確定要作廢此文件嗎 ? ')) {
      //   let self = this
      //   axios.post('/api/Document/Invalid', item).then((response) => {
      //     self.GetDocumentList()
      //   })
      // }
    },
    CancelInvalidPopup: function () {
      this.IsShowInvalidPopup = false
      this.InvalidDoc = null
      this.InvalidDate = ''
    },
    InvalidDocument: function () {
      let self = this

      let newEffDateStr = this.InvalidDate === '0001-01-01T00:00:00'
        ? ''
        : this.InvalidDate.split('T')[0]
      let newEffDate = new Date(newEffDateStr)
      let oldEffDate = new Date(this.InvalidDoc.EffectiveDate)

      if (newEffDate < oldEffDate) {
        self.AlertDialogMessage = '作廢日期必須大於原生效日期，請確認!!'
        self.IsOpenAlertDialog = true
        setTimeout(() => {
          self.IsOpenAlertDialog = false
        }, 1000)
      ...
      this.InvalidDoc.EffectiveDate = newEffDateStr

      axios.post('/api/Document/Invalid', this.InvalidDoc).then((response) => {
        self.GetDocumentList()
        self.IsShowInvalidPopup = false
      })
    },
    CancelVoidDocument: function (item) {
      if (confirm('確定要取消作廢此文件嗎 ? ')) {
        let self = this
        axios.post('/api/Document/CancelVoid', item).then((response) => {
          self.GetDocumentList()
        })
      }
    },
    ...
    GetDocumentVersionList: function (item) {
      let self = this
      self.versionHistoryIsLoading = true
      axios
        .post('/api/Document/GetDocumentVersionList', { DocumentId: item.Id })
        .then((response) => {
          self.DocumentVersionList = response.data
          self.versionHistoryIsLoading = false
          self.documentVersionListDialog = true
        })
    },
...
*/
    ...
    ChangeVersionEditDocumentTypes (typeId) {
      let prefixDocumentNumber = this.GetPrefixDocumentNumber(typeId)
      this.VersionEdit.DocumentNumber = prefixDocumentNumber
    },
